...
func fetchSessionForObject(ctx *context.AffinityContext, input Wrapper) (*session.Session, error) {
	...
	// Datacenter is very much necessary since we use the finder.
	params = params.WithDatacenter(template.Spec.Template.Spec.Datacenter)

	return fetchSession(ctx, params)
}

func newParams(ctx context.AffinityContext) *session.Params {
...
func fetchSession(ctx *context.AffinityContext, params *session.Params) (*session.Session, error) {
