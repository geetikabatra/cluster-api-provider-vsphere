...
func (r nodeLabelReconciler) reconcileNormal(ctx *nodeContext) (reconcile.Result, error) {
	...
	if err != nil {
		r.Logger.Info("The control plane is not ready yet", "err", err)
	...
	r.Logger.V(4).Info("Marked machine node with prefixed labels", "number-of-labels", len(nodePrefixLabels))
	return reconcile.Result{}, nil
}
